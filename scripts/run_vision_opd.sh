...
TEACHER_INPUT_MODE="${TEACHER_INPUT_MODE:-crop_only}"

case "${TEACHER_INPUT_MODE}" in
    crop_only|crop_plus_full) ;;
    *)
        echo "Unsupported TEACHER_INPUT_MODE: ${TEACHER_INPUT_MODE}" >&2
        exit 1
        ;;
esac
...
if [[ "${TEACHER_INPUT_MODE}" != "crop_only" ]]; then
    EXPERIMENT_NAME="${EXPERIMENT_NAME}-${TEACHER_INPUT_MODE}"
fi
...
echo "Teacher input mode: $TEACHER_INPUT_MODE"
...
python3 -m verl.trainer.main_ppo --config-name "$CONFIG_NAME" \
    ...
    actor_rollout_ref.actor.self_distillation.teacher_input_mode=$TEACHER_INPUT_MODE \
