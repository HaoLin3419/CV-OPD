...
This script:
1. Downloads train.jsonl from the dataset repository supplied with --hf-repo
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--hf-repo",
        default=os.environ.get("VISION_OPD_DATASET_REPO"),
        required=False,
        help="HuggingFace dataset repo (or set VISION_OPD_DATASET_REPO)",
    )
...
def main() -> None:
    ...
    if not args.skip_download and not args.hf_repo:
        raise SystemExit("--hf-repo or VISION_OPD_DATASET_REPO is required unless --skip-download is set")
