...
def compute_group_inverse_advantage_weights(
    advantages: torch.Tensor,
    response_mask: torch.Tensor,
    index: np.ndarray,
    temperature: float = 1.0,
) -> torch.Tensor:
    """Convert sequence-level GRPO advantages into detached inverse weights.

    For every rollout group, this implements
    ``exp(-A_i / tau) / mean_j(exp(-A_j / tau))``. GRPO outcome
    advantages are constant over valid response tokens, so the sequence value
    is recovered with a masked token mean before grouping.
    """
    if advantages.shape != response_mask.shape:
        raise ValueError(
            "advantages and response_mask must have the same shape, "
            f"got {tuple(advantages.shape)} and {tuple(response_mask.shape)}"
        )
    if advantages.ndim != 2:
        raise ValueError(f"advantages must have shape [batch, response_length], got {tuple(advantages.shape)}")
    if len(index) != advantages.shape[0]:
        raise ValueError(f"index length must match batch size, got {len(index)} and {advantages.shape[0]}")
    if temperature <= 0.0:
        raise ValueError(f"inverse advantage temperature must be positive, got {temperature}")
    ...
    with torch.no_grad():
        ...
        sequence_advantages = (advantages * mask).sum(dim=-1) / token_counts
        weights = torch.empty_like(sequence_advantages)

        group_rows: dict[Any, list[int]] = defaultdict(list)
        for row, group_id in enumerate(index):
            group_rows[group_id].append(row)
        for rows in group_rows.values():
            row_tensor = torch.as_tensor(rows, device=advantages.device, dtype=torch.long)
            logits = -sequence_advantages.index_select(0, row_tensor) / temperature
            # Subtracting the group maximum is algebraically neutral and avoids overflow.
            unnormalized = torch.exp(logits - logits.max())
            group_weights = unnormalized / unnormalized.mean()
            weights.index_copy_(0, row_tensor, group_weights)

    return weights.detach()


def compute_group_advantage_weights(
    advantages: torch.Tensor,
    response_mask: torch.Tensor,
    index: np.ndarray,
    temperature: float = 1.0,
) -> torch.Tensor:
    """Convert sequence-level GRPO advantages into detached positive weights.

    This is the v9-15 counterpart of the v9-9 inverse weighting rule:
    ``exp(+A_i / tau) / mean_j(exp(+A_j / tau))`` within each rollout group.
    """
    if advantages.shape != response_mask.shape:
        raise ValueError(
            "advantages and response_mask must have the same shape, "
    ...
    if len(index) != advantages.shape[0]:
        raise ValueError(f"index length must match batch size, got {len(index)} and {advantages.shape[0]}")
    if temperature <= 0.0:
        raise ValueError(f"advantage temperature must be positive, got {temperature}")
...
def build_crop_black_contrast_log_probs(
    crop_log_probs: torch.Tensor,
    black_log_probs: torch.Tensor,
    contrastive_alpha: float = 1.0,
    excluded_token_ids: Optional[list[int]] = None,
) -> torch.Tensor:
    if crop_log_probs.shape != black_log_probs.shape:
        raise ValueError(
            "crop and black teacher distributions must have identical shapes, "
            f"got {tuple(crop_log_probs.shape)} and {tuple(black_log_probs.shape)}"
        )
    if contrastive_alpha < 0.0:
        raise ValueError(f"contrastive_alpha must be non-negative, got {contrastive_alpha}")

    contrast_scores = crop_log_probs + contrastive_alpha * (crop_log_probs - black_log_probs)
    if excluded_token_ids:
        vocab_size = crop_log_probs.shape[-1]
        valid_ids = sorted({token_id for token_id in excluded_token_ids if 0 <= token_id < vocab_size})
        if valid_ids:
            contrast_scores = contrast_scores.clone()
            contrast_scores[..., valid_ids] = crop_log_probs[..., valid_ids]
    return torch.log_softmax(contrast_scores, dim=-1)


def compute_crop_black_contrast_forward_kl(
    ...
    if student_log_probs.shape != crop_log_probs.shape or crop_log_probs.shape != black_log_probs.shape:
        raise ValueError(
            "student, crop, and black distributions must have identical shapes, "
            f"got {tuple(student_log_probs.shape)}, {tuple(crop_log_probs.shape)}, "
            f"and {tuple(black_log_probs.shape)}"
        )
    if vocab_chunk_size <= 0:
        raise ValueError(f"vocab_chunk_size must be positive, got {vocab_chunk_size}")

    vocab_size = crop_log_probs.shape[-1]
    excluded_ids = sorted({token_id for token_id in (excluded_token_ids or []) if 0 <= token_id < vocab_size})
    compute_dtype = (
        torch.float32
        if crop_log_probs.dtype in (torch.float16, torch.bfloat16)
        else crop_log_probs.dtype
    )

    def contrast_scores(start: int, end: int) -> torch.Tensor:
        crop_chunk = crop_log_probs[..., start:end].to(compute_dtype)
        black_chunk = black_log_probs[..., start:end].to(compute_dtype)
        scores = crop_chunk + contrastive_alpha * (crop_chunk - black_chunk)
        local_excluded_ids = [token_id - start for token_id in excluded_ids if start <= token_id < end]
        if local_excluded_ids:
            scores = scores.clone()
            scores[..., local_excluded_ids] = crop_chunk[..., local_excluded_ids]
        return scores

    log_normalizer = torch.full(
        crop_log_probs.shape[:-1],
        float("-inf"),
        dtype=compute_dtype,
        device=crop_log_probs.device,
    )
    for start in range(0, vocab_size, vocab_chunk_size):
        end = min(start + vocab_chunk_size, vocab_size)
        chunk_logsumexp = torch.logsumexp(contrast_scores(start, end), dim=-1)
        log_normalizer = torch.logaddexp(log_normalizer, chunk_logsumexp)

    forward_kl = torch.zeros_like(log_normalizer)
    crop_black_kl = torch.zeros_like(log_normalizer)
    target_crop_kl = torch.zeros_like(log_normalizer)
    target_entropy = torch.zeros_like(log_normalizer)
    for start in range(0, vocab_size, vocab_chunk_size):
        end = min(start + vocab_chunk_size, vocab_size)
        crop_chunk = crop_log_probs[..., start:end].to(compute_dtype)
        black_chunk = black_log_probs[..., start:end].to(compute_dtype)
        student_chunk = student_log_probs[..., start:end].to(compute_dtype)
        target_log_chunk = contrast_scores(start, end) - log_normalizer.unsqueeze(-1)
        target_prob_chunk = target_log_chunk.exp()
        crop_prob_chunk = crop_chunk.exp()
        forward_kl = forward_kl + (
            target_prob_chunk * (target_log_chunk - student_chunk)
        ).sum(dim=-1)
        crop_black_kl += (crop_prob_chunk * (crop_chunk - black_chunk)).sum(dim=-1)
        target_crop_kl += (target_prob_chunk * (target_log_chunk - crop_chunk)).sum(dim=-1)
        target_entropy -= (target_prob_chunk * target_log_chunk).sum(dim=-1)

    return forward_kl, crop_black_kl, target_crop_kl, target_entropy


def compute_log_jsd(log_probs_a: torch.Tensor, log_probs_b: torch.Tensor) -> torch.Tensor:
    """Jensen-Shannon divergence for normalized log-probabilities."""
    if log_probs_a.shape != log_probs_b.shape:
        raise ValueError("JSD inputs must have identical shapes")
    dtype = torch.float32 if log_probs_a.dtype in (torch.float16, torch.bfloat16) else log_probs_a.dtype
    log_a = log_probs_a.to(dtype)
    log_b = log_probs_b.to(dtype)
    log_m = torch.logaddexp(log_a, log_b) - torch.log(torch.tensor(2.0, device=log_a.device, dtype=dtype))
    return 0.5 * ((log_a.exp() * (log_a - log_m)).sum(-1) + (log_b.exp() * (log_b - log_m)).sum(-1))


def add_topk_tail_log_probs(log_probs: torch.Tensor) -> torch.Tensor:
    """Append the probability mass outside a top-k support as one tail bucket."""
...
def compute_topk_tail_jsd(log_probs_a: torch.Tensor, log_probs_b: torch.Tensor) -> torch.Tensor:
    """Compute JSD on aligned top-k log-probabilities plus the shared tail bucket."""
    if log_probs_a.shape != log_probs_b.shape:
        raise ValueError("top-k JSD inputs must have identical shapes")
    return compute_log_jsd(add_topk_tail_log_probs(log_probs_a), add_topk_tail_log_probs(log_probs_b))


def compute_opdv_loss(
    student_topk_log_probs: torch.Tensor,
    positive_topk_log_probs: torch.Tensor,
    positive_sampled_log_probs: torch.Tensor,
    negative_sampled_log_probs: torch.Tensor,
    response_mask: torch.Tensor,
    *,
    batch_num_tokens: Optional[int | float | torch.Tensor] = None,
) -> tuple[torch.Tensor, dict[str, torch.Tensor]]:
    """Compute OPD-V on positive-margin sampled tokens and student Top-K support."""
    if student_topk_log_probs.shape != positive_topk_log_probs.shape:
        raise ValueError("OPD-V student and positive Top-K distributions must have identical shapes")
    token_shape = student_topk_log_probs.shape[:-1]
    if (
        positive_sampled_log_probs.shape != token_shape
        or negative_sampled_log_probs.shape != token_shape
        or response_mask.shape != token_shape
    ):
        raise ValueError("OPD-V sampled log-probabilities and response mask must match Top-K token dimensions")

    margin = (positive_sampled_log_probs.detach() - negative_sampled_log_probs.detach()).detach()
    gate = (response_mask > 0) & (margin > 0)
    token_jsd = compute_topk_tail_jsd(
        student_topk_log_probs, positive_topk_log_probs.detach()
    )
    per_token = margin * token_jsd * gate.to(token_jsd.dtype)
    denominator = response_mask.sum() if batch_num_tokens is None else batch_num_tokens
    denominator = torch.as_tensor(
        denominator, device=per_token.device, dtype=per_token.dtype
    ).clamp_min(1.0)
    loss = per_token.sum() / denominator
    return loss, {
        "margin": margin,
        "gate": gate,
        "token_jsd": token_jsd,
...
def build_batch_mean_jsd_gate(
    token_jsd: torch.Tensor,
    valid_mask: torch.Tensor,
    *,
    distributed: bool = True,
) -> tuple[torch.Tensor, torch.Tensor]:
    """Return strict-above-mean gates over valid response tokens."""
    if token_jsd.shape != valid_mask.shape:
        raise ValueError("token_jsd and valid_mask must have identical shapes")
    valid = valid_mask > 0
    total = token_jsd.masked_select(valid).sum(dtype=torch.float64)
    count = valid.sum().to(dtype=torch.float64)
    if distributed and torch.distributed.is_available() and torch.distributed.is_initialized():
        torch.distributed.all_reduce(total, op=torch.distributed.ReduceOp.SUM)
        torch.distributed.all_reduce(count, op=torch.distributed.ReduceOp.SUM)
    threshold = (total / count.clamp(min=1.0)).to(token_jsd.dtype)
    return valid & (token_jsd > threshold), threshold


def build_per_response_mean_jsd_gate(
    token_jsd: torch.Tensor,
    valid_mask: torch.Tensor,
) -> tuple[torch.Tensor, torch.Tensor]:
    """Gate tokens strictly above the mean JSD of their own response.

    Unlike ``build_batch_mean_jsd_gate``, each response row gets an
    independent threshold. Padding/invalid response positions are excluded;
    rows with no valid tokens receive a zero threshold and an all-false gate.
    """
    if token_jsd.shape != valid_mask.shape:
        raise ValueError("token_jsd and valid_mask must have identical shapes")
    valid = valid_mask > 0
    valid_float = valid.to(dtype=token_jsd.dtype)
    counts = valid_float.sum(dim=-1, keepdim=True)
    thresholds = (token_jsd * valid_float).sum(dim=-1, keepdim=True) / counts.clamp(min=1.0)
    thresholds = thresholds.squeeze(-1)
    return valid & (token_jsd > thresholds.unsqueeze(-1)), thresholds


def build_batch_mean_multi_negative_probability_gate(
    token_probability_gaps: torch.Tensor,
    valid_mask: torch.Tensor,
    *,
    distributed: bool = True,
) -> tuple[torch.Tensor, torch.Tensor]:
    """Gate on each token's two smallest negative-teacher probability gaps.

    The threshold is the mean of every valid token gap from every negative
    teacher, rather than the mean of the per-token selected gaps.
    """
    if token_probability_gaps.ndim != valid_mask.ndim + 1:
        raise ValueError("token_probability_gaps must have one negative-teacher dimension")
    if token_probability_gaps.shape[:-1] != valid_mask.shape:
        raise ValueError("token_probability_gaps and valid_mask have incompatible shapes")
    negative_count = token_probability_gaps.shape[-1]
    if negative_count < 2:
        raise ValueError("at least two negative teachers are required")

    valid = valid_mask > 0
    selected_gap = torch.topk(token_probability_gaps, k=2, dim=-1, largest=False).values.mean(dim=-1)
    total = token_probability_gaps.masked_select(valid.unsqueeze(-1)).sum(dtype=torch.float64)
    count = valid.sum(dtype=torch.float64) * negative_count
    if distributed and torch.distributed.is_available() and torch.distributed.is_initialized():
        torch.distributed.all_reduce(total, op=torch.distributed.ReduceOp.SUM)
        torch.distributed.all_reduce(count, op=torch.distributed.ReduceOp.SUM)
    threshold = (total / count.clamp(min=1.0)).to(token_probability_gaps.dtype)
    return valid & (selected_gap > threshold), threshold


def compute_crop_full_abs_contrast_forward_kl(
    student_log_probs: torch.Tensor,
    crop_log_probs: torch.Tensor,
    ...
    """Compute KL(sg[q*] || student), q* proportional to crop exp(alpha |crop-full|)."""
    if student_log_probs.shape != crop_log_probs.shape or crop_log_probs.shape != full_log_probs.shape:
        raise ValueError("student, crop, and full distributions must have identical shapes")
    if contrastive_alpha < 0.0 or vocab_chunk_size <= 0:
        raise ValueError("contrastive_alpha must be non-negative and vocab_chunk_size must be positive")
    dtype = torch.float32 if crop_log_probs.dtype in (torch.float16, torch.bfloat16) else crop_log_probs.dtype
    vocab_size = crop_log_probs.shape[-1]

    def score_chunk(start: int, end: int) -> torch.Tensor:
        crop = crop_log_probs[..., start:end].to(dtype)
        full = full_log_probs[..., start:end].to(dtype)
        return crop + contrastive_alpha * (crop - full).abs()

    log_z = torch.full(crop_log_probs.shape[:-1], float("-inf"), dtype=dtype, device=crop_log_probs.device)
    for start in range(0, vocab_size, vocab_chunk_size):
        end = min(start + vocab_chunk_size, vocab_size)
        log_z = torch.logaddexp(log_z, torch.logsumexp(score_chunk(start, end), dim=-1))

    forward_kl = torch.zeros_like(log_z)
    target_crop_kl = torch.zeros_like(log_z)
    expected_abs_gap = torch.zeros_like(log_z)
    for start in range(0, vocab_size, vocab_chunk_size):
        end = min(start + vocab_chunk_size, vocab_size)
        crop = crop_log_probs[..., start:end].to(dtype)
        full = full_log_probs[..., start:end].to(dtype)
        target_log = score_chunk(start, end) - log_z.unsqueeze(-1)
        target_prob = target_log.exp()
        student = student_log_probs[..., start:end].to(dtype)
        forward_kl += (target_prob * (target_log - student)).sum(-1)
        target_crop_kl += (target_prob * (target_log - crop)).sum(-1)
        expected_abs_gap += (crop.exp() * (crop - full).abs()).sum(-1)
    return forward_kl, target_crop_kl, expected_abs_gap


def compute_crop_forward_kl(
    student_log_probs: torch.Tensor,
    crop_log_probs: torch.Tensor,
) -> torch.Tensor:
    """Compute ordinary forward KL from the positive-crop teacher to student."""
    if student_log_probs.shape != crop_log_probs.shape:
        raise ValueError("student and crop distributions must have identical shapes")
    return F.kl_div(
        student_log_probs,
        crop_log_probs.detach(),
        reduction="none",
        log_target=True,
    ).sum(-1)


def compute_sampled_token_probability_ranking_loss(
    student_log_probs: torch.Tensor,
    ...
    """Rank positive and negative teachers using absolute sampled-token probability gaps."""
    if margin < 0.0:
        raise ValueError(f"margin must be non-negative, got {margin}")
    if student_log_probs.ndim != 2 or positive_log_probs.shape != student_log_probs.shape or negative_log_probs.shape != student_log_probs.shape:
        raise ValueError("ranking distributions must all have shape [tokens, vocab]")
    if sampled_token_ids.shape != (student_log_probs.shape[0],):
        raise ValueError("sampled_token_ids must have shape [tokens]")
    token_ids = sampled_token_ids.to(device=student_log_probs.device, dtype=torch.long).unsqueeze(-1)
    vocab_size = student_log_probs.shape[-1]
    if token_ids.numel() and (token_ids.min() < 0 or token_ids.max() >= vocab_size):
        raise ValueError("sampled_token_ids contain an out-of-range vocabulary index")
    student_prob = student_log_probs.gather(-1, token_ids).squeeze(-1).exp()
    positive_prob = positive_log_probs.detach().gather(-1, token_ids).squeeze(-1).exp()
    negative_prob = negative_log_probs.detach().gather(-1, token_ids).squeeze(-1).exp()
    positive_abs_diff = (student_prob - positive_prob).abs()
    negative_abs_diff = (student_prob - negative_prob).abs()
    per_token = torch.relu(margin + positive_abs_diff - negative_abs_diff)
    loss = per_token.mean() if per_token.numel() else student_log_probs.sum() * 0.0
    return loss, {"positive_abs_diff": positive_abs_diff, "negative_abs_diff": negative_abs_diff, "per_token": per_token}

def compute_jsd_ranking_loss(
    student_log_probs: torch.Tensor,
    positive_log_probs: torch.Tensor,
    negative_log_probs: torch.Tensor,
    margin: float = 0.1,
    ...
    if margin < 0.0:
        raise ValueError(f"margin must be non-negative, got {margin}")
    positive_jsd = compute_log_jsd(student_log_probs, positive_log_probs)
    negative_jsd = compute_log_jsd(student_log_probs, negative_log_probs)
    per_token = torch.relu(margin + positive_jsd - negative_jsd)
    loss = per_token.mean() if per_token.numel() else student_log_probs.sum() * 0.0
    return loss, {"positive_jsd": positive_jsd, "negative_jsd": negative_jsd, "per_token": per_token}


def compute_teacher_trajectory_sft_loss(
    student_log_probs: torch.Tensor,
    response_mask: torch.Tensor,
    teacher_trajectory_mask: torch.Tensor,
    *,
    global_valid_token_count: Optional[int | float | torch.Tensor] = None,
    dp_size: int = 1,
) -> tuple[torch.Tensor, dict[str, Any]]:
    """Token-mean SFT used only by correct v9-13 crop-teacher trajectories."""
    if student_log_probs.shape != response_mask.shape:
        raise ValueError("student_log_probs and response_mask must have identical shapes")
    if teacher_trajectory_mask.ndim != 1 or teacher_trajectory_mask.shape[0] != response_mask.shape[0]:
        raise ValueError(
            "teacher_trajectory_mask must have shape [batch], "
            f"got {tuple(teacher_trajectory_mask.shape)} for {tuple(response_mask.shape)}"
        )
    loss_mask = response_mask * teacher_trajectory_mask.to(
        device=response_mask.device, dtype=response_mask.dtype
    ).unsqueeze(1)
    local_valid_tokens = loss_mask.sum()
    denominator = local_valid_tokens if global_valid_token_count is None else global_valid_token_count
    denominator = torch.as_tensor(denominator, device=student_log_probs.device, dtype=student_log_probs.dtype)
    denominator = denominator.clamp_min(1.0)
    loss = -verl_F.masked_sum(student_log_probs, loss_mask) / denominator * dp_size
    metrics = {
        "self_distillation/teacher_sft_loss": loss.detach().item(),
        "self_distillation/teacher_sft_trajectories": teacher_trajectory_mask.sum().detach().item(),
        "self_distillation/teacher_sft_tokens": local_valid_tokens.detach().item(),
    }
    return loss, metrics
...
def compute_self_distillation_loss(
    ...
    reference_teacher_all_log_probs: Optional[torch.Tensor] = None,
    negative_teacher_gated_log_probs: Optional[torch.Tensor] = None,
    opdv_positive_sampled_log_probs: Optional[torch.Tensor] = None,
    opdv_negative_sampled_log_probs: Optional[torch.Tensor] = None,
    sampled_response_token_ids: Optional[torch.Tensor] = None,
    ranking_gate: Optional[torch.Tensor] = None,
    ranking_batch_token_count: Optional[int] = None,
    contrastive_excluded_token_ids: Optional[list[int]] = None,
    self_distillation_mask: Optional[torch.Tensor] = None,
    self_distillation_sample_filter: Optional[torch.Tensor] = None,
    loss_agg_mode: str = "token-mean",
    rollout_is_weights: Optional[torch.Tensor] = None,
    batch_num_tokens: Optional[int] = None,
    global_batch_size: Optional[int] = None,
    loss_scale_factor: Optional[int] = None,
    self_distillation_wrong_mask: Optional[torch.Tensor] = None,
    inverse_advantage_weights: Optional[torch.Tensor] = None,
) -> tuple[torch.Tensor, dict[str, Any]]:

    metrics = {}
    distillation_target_mode = self_distillation_config.get("distillation_target_mode", "standard")

    hybrid_auxiliary_modes = {
        "crop_only_v8_1",
        "crop_only_v8_2",
        "crop_only_v8_3",
        "crop_only_v8_4",
    ...
    sampled_probability_auxiliary_modes = {
        "crop_only_v8_2",
        "crop_only_v8_3",
        "crop_only_v8_4",
        "crop_only_v8_3_2",
    ...
    weighted_sampled_gap_auxiliary_modes = {
        "crop_only_v8_3",
        "crop_only_v8_4",
        "crop_only_v8_5",
        "crop_only_v8_6",
    ...
    if distillation_target_mode in {"crop_only_v9_11", "crop_only_v9_12", "crop_only_v9_13", "crop_only_v9_14", "crop_only_v10_2"}:
        if self_distillation_sample_filter is None:
            raise ValueError(f"{distillation_target_mode} requires a sample-level distillation filter")
        if self_distillation_sample_filter.ndim != 1 or self_distillation_sample_filter.shape[0] != loss_mask.shape[0]:
            raise ValueError(
                f"{distillation_target_mode} sample-level filter must have shape [batch], "
                f"got {tuple(self_distillation_sample_filter.shape)} for loss mask {tuple(loss_mask.shape)}"
            )
        loss_mask = loss_mask * self_distillation_sample_filter.to(
            device=loss_mask.device, dtype=loss_mask.dtype
        ).unsqueeze(1)
    if distillation_target_mode == "crop_only_v9_8":
        if self_distillation_wrong_mask is None:
            raise ValueError("crop_only_v9_8 requires a wrong-trajectory distillation mask")
        if self_distillation_wrong_mask.ndim != 1 or self_distillation_wrong_mask.shape[0] != loss_mask.shape[0]:
            raise ValueError(
                "crop_only_v9_8 wrong-trajectory mask must have shape [batch], "
                f"got {tuple(self_distillation_wrong_mask.shape)} for loss mask {tuple(loss_mask.shape)}"
            )
        loss_mask = loss_mask * self_distillation_wrong_mask.to(
            device=loss_mask.device, dtype=loss_mask.dtype
        ).unsqueeze(1)
    if distillation_target_mode == "crop_only_v9_2":
        if ranking_gate is None:
            raise ValueError("crop_only_v9_2 requires a wrong-trajectory batch-mean JSD distillation gate")
        if ranking_gate.shape != loss_mask.shape:
            raise ValueError(
                "crop_only_v9_2 ranking_gate must match response_mask shape, "
                f"got {tuple(ranking_gate.shape)} and {tuple(loss_mask.shape)}"
            )
        loss_mask = loss_mask * (ranking_gate.to(device=loss_mask.device) > 0).to(loss_mask.dtype)
    ...
    if self_distillation_config.full_logit_distillation:
        ...
        if distillation_target_mode in {
            ...
            if use_topk:
                raise ValueError("crop_full_abs_contrast_rank modes require full-vocabulary distributions")
            if distillation_target_mode == "crop_full_abs_contrast_rank" and reference_teacher_all_log_probs is None:
                raise ValueError("crop_full_abs_contrast_rank requires the full-image teacher distribution")
            if distillation_target_mode in {
                "crop_full_abs_contrast_rank_v2",
                "crop_full_abs_contrast_rank_v3",
                "crop_full_abs_contrast_rank_v4",
            ...
            "crop_full_abs_contrast_rank_v7",
                ...
                raw_per_token_loss = compute_crop_forward_kl(
                    student_distill_log_probs, teacher_distill_log_probs
                )
                target_crop_kl = raw_per_token_loss
                abs_teacher_gap = None
            else:
                raw_per_token_loss, target_crop_kl, abs_teacher_gap = compute_crop_full_abs_contrast_forward_kl(
                    student_distill_log_probs,
                    teacher_distill_log_probs,
                    reference_teacher_all_log_probs,
                    contrastive_alpha=self_distillation_config.get("contrastive_alpha", 1.0),
                    vocab_chunk_size=self_distillation_config.get("contrastive_vocab_chunk_size", 8192),
                )
            temperature = self_distillation_config.get("distillation_temperature", 1.0)
            raw_per_token_loss = raw_per_token_loss * (temperature ** 2)
            valid_token_count = loss_mask.sum().clamp(min=1.0)
            metrics["self_distillation/positive_crop_kl_token_mean"] = (
                verl_F.masked_sum(target_crop_kl, loss_mask) / valid_token_count
            ).detach().item()
            if abs_teacher_gap is not None:
                metrics["self_distillation/abs_contrast_target_crop_kl_token_mean"] = metrics[
                    "self_distillation/positive_crop_kl_token_mean"
                ]
                metrics["self_distillation/abs_teacher_gap_token_mean"] = (
                    verl_F.masked_sum(abs_teacher_gap, loss_mask) / valid_token_count
                ).detach().item()
            if ranking_gate is not None and negative_teacher_gated_log_probs is not None:
                gated = (ranking_gate > 0) & (loss_mask > 0)
                gated_student = student_distill_log_probs[gated]
                gated_positive = teacher_distill_log_probs[gated]
                if negative_teacher_gated_log_probs.shape != gated_student.shape:
                    raise ValueError(
                        "negative gated distributions must match gated student rows, "
                        f"got {tuple(negative_teacher_gated_log_probs.shape)} and {tuple(gated_student.shape)}"
                    )
                if gated.any():
                    if distillation_target_mode in {
                        "crop_full_abs_contrast_rank_v5",
                        ...
                        if sampled_response_token_ids is None:
                            raise ValueError("crop_full_abs_contrast_rank_v5 or v6 requires sampled_response_token_ids")
                        gated_token_ids = sampled_response_token_ids[gated]
                        _, rank_parts = compute_sampled_token_probability_ranking_loss(
                            gated_student,
                            gated_positive,
                            negative_teacher_gated_log_probs,
                            gated_token_ids,
                            margin=self_distillation_config.get("ranking_margin", 0.1),
                        )
                    else:
                        _, rank_parts = compute_jsd_ranking_loss(
                            gated_student,
                            gated_positive,
                            negative_teacher_gated_log_probs,
                            margin=self_distillation_config.get("ranking_margin", 0.1),
                        )
                    if ranking_batch_token_count is None or ranking_batch_token_count <= 0:
                        raise ValueError("ranking_batch_token_count must be positive when ranking is active")
                    if batch_num_tokens is None:
                        raise ValueError("batch_num_tokens is required for exact ranking normalization")
                    ranking_scale = float(batch_num_tokens) / float(ranking_batch_token_count)
                    raw_per_token_loss = raw_per_token_loss.clone()
                    raw_per_token_loss[gated] += (
                        self_distillation_config.get("ranking_loss_weight", 0.1)
                        * ranking_scale
                        * rank_parts["per_token"]
                    )
                    metrics["self_distillation/ranking_loss"] = rank_parts["per_token"].mean().detach().item()
                    if distillation_target_mode in {
                        "crop_full_abs_contrast_rank_v5",
                        "crop_full_abs_contrast_rank_v6",
                    }:
                        metrics["self_distillation/ranking_positive_probability_abs_diff"] = rank_parts["positive_abs_diff"].mean().detach().item()
                        metrics["self_distillation/ranking_negative_probability_abs_diff"] = rank_parts["negative_abs_diff"].mean().detach().item()
                    else:
                        metrics["self_distillation/ranking_positive_jsd"] = rank_parts["positive_jsd"].mean().detach().item()
                        metrics["self_distillation/ranking_negative_jsd"] = rank_parts["negative_jsd"].mean().detach().item()
                metrics["self_distillation/ranking_gated_tokens"] = gated.sum().detach().item()
                metrics["self_distillation/ranking_selected_tokens"] = gated.sum().detach().item()
        elif distillation_target_mode == "crop_black_contrast":
            if use_topk:
                raise ValueError("crop_black_contrast requires full-vocabulary distributions.")
            if reference_teacher_all_log_probs is None:
                raise ValueError("crop_black_contrast requires reference_teacher_all_log_probs.")
            (
                raw_per_token_loss,
                crop_black_kl,
                target_crop_kl,
                target_entropy,
            ) = compute_crop_black_contrast_forward_kl(
                student_distill_log_probs,
                teacher_distill_log_probs,
                reference_teacher_all_log_probs,
                contrastive_alpha=self_distillation_config.get("contrastive_alpha", 1.0),
                excluded_token_ids=contrastive_excluded_token_ids,
                vocab_chunk_size=self_distillation_config.get("contrastive_vocab_chunk_size", 8192),
            )
            distillation_temperature = self_distillation_config.get("distillation_temperature", 1.0)
            raw_per_token_loss = raw_per_token_loss * (distillation_temperature**2)
            with torch.no_grad():
                valid_token_count = loss_mask.sum().clamp(min=1.0)
                metrics["self_distillation/crop_black_kl_token_mean"] = (
                    verl_F.masked_sum(crop_black_kl, loss_mask) / valid_token_count
                ).item()
                metrics["self_distillation/contrast_target_crop_kl_token_mean"] = (
                    verl_F.masked_sum(target_crop_kl, loss_mask) / valid_token_count
                ).item()
                metrics["self_distillation/contrast_target_entropy_token_mean"] = (
                    verl_F.masked_sum(target_entropy, loss_mask) / valid_token_count
                ).item()
        ...
        if distillation_target_mode not in {
            "crop_black_contrast",
            "crop_full_abs_contrast_rank",
            "crop_full_abs_contrast_rank_v2",
            "crop_full_abs_contrast_rank_v3",
        ...
        if distillation_target_mode in {"crop_only_v9_9", "crop_only_v9_15"}:
            if inverse_advantage_weights is None:
                raise ValueError(f"{distillation_target_mode} requires advantage weights")
            if inverse_advantage_weights.ndim != 1 or inverse_advantage_weights.shape[0] != loss_mask.shape[0]:
                raise ValueError(
                    f"{distillation_target_mode} advantage weights must have shape [batch], "
                    f"got {tuple(inverse_advantage_weights.shape)} for loss mask {tuple(loss_mask.shape)}"
                )
            detached_weights = inverse_advantage_weights.to(
                device=raw_per_token_loss.device, dtype=raw_per_token_loss.dtype
            ).detach()
            # Apply the detached main-distillation coefficient before the unchanged
            # auxiliary term is added below, so ranking is never reweighted.
            raw_per_token_loss = raw_per_token_loss * detached_weights.unsqueeze(1)
            metrics["self_distillation/inverse_advantage_weight_mean"] = detached_weights.mean().item()
            metrics["self_distillation/inverse_advantage_weight_min"] = detached_weights.min().item()
            metrics["self_distillation/inverse_advantage_weight_max"] = detached_weights.max().item()
        if distillation_target_mode in hybrid_auxiliary_modes:
            # All v8 modes keep the original Top-K+tail JSD as the main loss.
            # Existing v8 ranking uses full-vocabulary distributions; v9-1 uses
            # the student's aligned Top-K+tail support for every ranking JSD.
            if distillation_target_mode == "crop_only_v9_1":
                if student_topk_log_probs is None or teacher_topk_log_probs is None:
                    raise ValueError("crop_only_v9_1 requires student and positive-teacher top-k distributions")
            elif student_all_log_probs is None or teacher_all_log_probs is None:
                raise ValueError(f"{distillation_target_mode} requires full-vocabulary student and positive-teacher distributions")
            if ranking_gate is not None and negative_teacher_gated_log_probs is not None:
                gated = (ranking_gate > 0) & (loss_mask > 0)
                if distillation_target_mode == "crop_only_v9_1":
                    gated_student_topk = student_topk_log_probs[gated]
                    gated_positive_topk = teacher_topk_log_probs[gated]
                    if negative_teacher_gated_log_probs.shape != gated_student_topk.shape:
                        raise ValueError(
                            "negative gated top-k distributions must match selected student rows, "
                            f"got {tuple(negative_teacher_gated_log_probs.shape)} and {tuple(gated_student_topk.shape)}"
                        )
                    gated_student = add_topk_tail_log_probs(gated_student_topk)
                    gated_positive = add_topk_tail_log_probs(gated_positive_topk)
                    gated_negative = add_topk_tail_log_probs(negative_teacher_gated_log_probs)
                else:
                    gated_student = student_all_log_probs[gated]
                    gated_positive = teacher_all_log_probs[gated]
                    if negative_teacher_gated_log_probs.shape != gated_student.shape:
                        raise ValueError(
                            "negative gated distributions must match gated student rows, "
                            f"got {tuple(negative_teacher_gated_log_probs.shape)} and {tuple(gated_student.shape)}"
                        )
                    gated_negative = negative_teacher_gated_log_probs
                if gated.any():
                    if distillation_target_mode in sampled_probability_auxiliary_modes:
                        if sampled_response_token_ids is None:
                            raise ValueError(f"{distillation_target_mode} requires sampled_response_token_ids")
                        gated_token_ids = sampled_response_token_ids[gated]
                        _, rank_parts = compute_sampled_token_probability_ranking_loss(
                            gated_student, gated_positive, negative_teacher_gated_log_probs,
                            gated_token_ids, margin=self_distillation_config.get("ranking_margin", 0.1),
                        )
                        metrics["self_distillation/ranking_positive_probability_abs_diff"] = rank_parts["positive_abs_diff"].mean().detach().item()
                        metrics["self_distillation/ranking_negative_probability_abs_diff"] = rank_parts["negative_abs_diff"].mean().detach().item()
                    else:
                        _, rank_parts = compute_jsd_ranking_loss(
                            gated_student, gated_positive, gated_negative,
                            margin=self_distillation_config.get("ranking_margin", 0.1),
                        )
                        metrics["self_distillation/ranking_positive_jsd"] = rank_parts["positive_jsd"].mean().detach().item()
                        metrics["self_distillation/ranking_negative_jsd"] = rank_parts["negative_jsd"].mean().detach().item()
                    ...
                    if batch_num_tokens is None:
                        raise ValueError("batch_num_tokens is required for exact ranking normalization")
                    ranking_scale = float(batch_num_tokens) / float(ranking_batch_token_count)
                    auxiliary_per_token = rank_parts["per_token"]
                    if distillation_target_mode in weighted_sampled_gap_auxiliary_modes:
                        if sampled_response_token_ids is None:
                            raise ValueError(f"{distillation_target_mode} requires sampled_response_token_ids for weighting")
                        gated_token_ids = sampled_response_token_ids[gated]
                        token_ids = gated_token_ids.to(device=gated_positive.device, dtype=torch.long).unsqueeze(-1)
                        positive_probability = gated_positive.detach().gather(-1, token_ids).squeeze(-1).exp()
                        if distillation_target_mode in {
                            "crop_only_v8_3",
                            "crop_only_v8_5",
                            "crop_only_v8_3_2",
                            ...
                            comparison_probability = (
                                negative_teacher_gated_log_probs.detach().gather(-1, token_ids).squeeze(-1).exp()
                            )
                        else:
                            if reference_teacher_all_log_probs is None:
                                raise ValueError(f"{distillation_target_mode} requires the original-teacher distribution")
                            gated_original = reference_teacher_all_log_probs[gated]
                            if gated_original.shape != gated_positive.shape:
                                raise ValueError(
                                    "original-teacher distributions must match selected positive-teacher rows, "
                                    f"got {tuple(gated_original.shape)} and {tuple(gated_positive.shape)}"
                                )
                            comparison_probability = (
                                gated_original.detach().gather(-1, token_ids).squeeze(-1).exp()
                            )
                        teacher_probability_weight = (positive_probability - comparison_probability).abs()
                        auxiliary_per_token = auxiliary_per_token * teacher_probability_weight
                        metrics["self_distillation/ranking_teacher_probability_weight"] = (
                            teacher_probability_weight.mean().detach().item()
                        )
                        metrics["self_distillation/ranking_unweighted_loss"] = (
                            rank_parts["per_token"].mean().detach().item()
                        )
                    raw_per_token_loss = raw_per_token_loss.clone()
                    # The -2 variants intentionally remove the extra fixed 0.1
                    # coefficient; v8-7 keeps its original fixed coefficient.
                    ranking_loss_weight = (
                        1.0
                        if distillation_target_mode in {
                            "crop_only_v8_3_2",
                            ...
                            0.1
                            if distillation_target_mode in {"crop_only_v8_7", "crop_only_v9_2"}
                            else self_distillation_config.get("ranking_loss_weight", 0.1)
                        )
                    )
                    raw_per_token_loss[gated] += ranking_loss_weight * ranking_scale * auxiliary_per_token
                    metrics["self_distillation/ranking_loss"] = auxiliary_per_token.mean().detach().item()
                if distillation_target_mode in {"crop_only_v8_1", "crop_only_v8_2", "crop_only_v9_1", "crop_only_v9_2", "crop_only_v9_3", "crop_only_v9_5", "crop_only_v9_6", "crop_only_v9_7", "crop_only_v9_8", "crop_only_v9_9", "crop_only_v9_10", "crop_only_v9_11", "crop_only_v9_12", "crop_only_v9_13", "crop_only_v9_14", "crop_only_v9_15", "crop_only_v10_1", "crop_only_v10_2"}:
                    metrics["self_distillation/ranking_gated_tokens"] = gated.sum().detach().item()
                metrics["self_distillation/ranking_selected_tokens"] = gated.sum().detach().item()
    else:
    ...
    if distillation_target_mode == "crop_only_v10_1":
        if student_topk_log_probs is None or teacher_topk_log_probs is None:
            raise ValueError("crop_only_v10_1 requires student and positive-teacher Top-100 distributions")
        if opdv_positive_sampled_log_probs is None or opdv_negative_sampled_log_probs is None:
            raise ValueError("crop_only_v10_1 requires positive and negative sampled-token log-probabilities")
        opdv_loss, opdv_parts = compute_opdv_loss(
            student_topk_log_probs=student_topk_log_probs,
            positive_topk_log_probs=teacher_topk_log_probs,
            positive_sampled_log_probs=opdv_positive_sampled_log_probs,
            negative_sampled_log_probs=opdv_negative_sampled_log_probs,
            response_mask=loss_mask,
            batch_num_tokens=batch_num_tokens,
        )
        valid_opdv_tokens = loss_mask.sum().clamp_min(1.0)
        metrics["self_distillation/opdv_loss"] = opdv_loss.detach().item()
        metrics["self_distillation/opdv_gated_tokens"] = opdv_parts["gate"].sum().detach().item()
        metrics["self_distillation/opdv_gate_fraction"] = (
            opdv_parts["gate"].sum() / valid_opdv_tokens
        ).detach().item()
        metrics["self_distillation/opdv_positive_margin_mean"] = (
            opdv_parts["margin"].clamp_min(0.0) * opdv_parts["gate"].to(opdv_parts["margin"].dtype)
        ).sum().div(opdv_parts["gate"].sum().clamp_min(1)).detach().item()
    ...
    if opdv_loss is not None:
        loss = loss + 1.0 * opdv_loss
    return loss, metrics
