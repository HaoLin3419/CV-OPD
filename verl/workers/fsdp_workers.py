...
class ActorRolloutRefWorker(Worker, DistProfilerExtension):
            ...
            if self._is_actor:
                ...
                if self_distillation_cfg is not None and loss_mode == "vopd":
                    ...
                    if teacher_model_source == "current":
                        ...
                        configured_ref_path = self.config.ref.get("model", {}).get("path", self.config.model.path)
                        if str(configured_ref_path) == str(teacher_model_path):
                            # Reuse the colocated fixed teacher loaded as ref to avoid a duplicate 9B copy.
    ...
    def update_actor(self, data: DataProto):
        ...
        with self.ulysses_sharding_manager:
            ...
            if not data.meta_info.get("v9_13_teacher_sft_batch", False):
    ...
    @register(dispatch_mode=make_nd_compute_dataproto_dispatch_fn(mesh_name="actor"))
    @DistProfiler.annotate(color="purple", role="crop_teacher_generate")
    def generate_crop_teacher_trajectories(self, prompts: DataProto):
        """Reuse the colocated EMA teacher for v9-13 autoregressive generation."""
        assert self._is_actor
        teacher_module = self.actor.teacher_module
        if self._is_offload_param:
            load_fsdp_model_to_gpu(self.actor_module_fsdp)
            if isinstance(teacher_module, FSDP) and teacher_module is not self.actor_module_fsdp:
                load_fsdp_model_to_gpu(teacher_module)
        with self.ulysses_sharding_manager:
            output = self.actor.generate_crop_teacher_trajectories(prompts.to("cpu"))
        output = output.to("cpu")
        if self._is_offload_param:
            offload_fsdp_model_to_cpu(self.actor_module_fsdp)
            if isinstance(teacher_module, FSDP) and teacher_module is not self.actor_module_fsdp:
